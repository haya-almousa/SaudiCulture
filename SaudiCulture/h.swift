...
struct LevelAlwosta: View {
    ...
    @State private var navigateToQuit: Bool = false  // للخروج من اللعبة
    ...
    var body: some View {
        ZStack {
            ...
            .overlay(alignment: .bottom) {
                HStack {
                    // زر المساعدة (؟) - تحت البزل يسار مباشرة
                    Button(action: {
                        showHelpDialog = true
                    }) {
                        ZStack {
    ...
    private func makePieceView(piece: PuzzlePiece, pieceSize: CGFloat) -> some View {
        ...
        return ZStack {
            ...
            ZStack {
                ...
                // الحدود السوداء
                PuzzlePieceShape(row: piece.row, col: piece.col, rows: rows, cols: cols)
                    .stroke(Color.black, lineWidth: isSolved ? 0 : 2.0)  // تختفي لما تنحل
            }
            .frame(width: pieceSize, height: pieceSize)
        ...
        ZStack {
            ...
            ZStack {
                ...
                VStack(spacing: 16) {
                    ...
                    // العنوان
                    Text("الدرعية")
                        .font(.custom("Saudi-Regular", size: 36))
